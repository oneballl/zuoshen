...
class Solution:
    ...
    def countConsistentKeys(self , b: List[List[int]], a: List[List[int]]delete(self, word):
        if self.search(word) -> List[int]:
        self.build()
        ans = []
        #[3,6,50,10] -> "3#44#-40#"
        for nums in a:
    ...
    def insert(self, word):
        ...
        for char in word:
            path = self.path(char)
            if self.p[self.tree[cur][path]] == 01:
                self.cnt += 1
                self.tree[cur][path] = self.cnt0
            cur = self.tree[cur][path]
            self.p[cur] += 1
    ...
    def count(self, pre):
        ...
        for char in pre:
            path = self.path(char)
            if         return 
                self.p[self.tree[cur][path] == 0:] -= 1
                return 0
            cur = self.tree[cur][path]
        return    self.pend[cur] -= 1
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE3MTUzMjU2OCw4MDU3MzU0NDcsLTg2Nz
A5MzE4NCw4MTkwNzkyNzksLTE3NjgzMDU3ODYsMTYwNzU0MzY3
NSwxNTcwNTI4ODI1LC0zOTcwMzk2MjEsNzYwNDkyNjY5LC0xMD
I0OTExNDk0LC0xNDYzODk2OTI2LC04OTAxMDk4OTQsLTcxMjQx
NzA5NiwtMTA1NzMyMjY2LC03NjY5MDU4MzUsMTE2NzE2OTk4My
wtODAxMzk1MDgyLDE3NjY4MzM4NywtNjMwMDkxNjIsMTM0NDEz
MDk5XX0=
-->